...
###############################################################################
# LLM-only EM run with the same configuration as Tushar's best run
# (job 14713446), but using run_em_algorithm_llm_only.py.
#
# This is identical to em_replicate_best.sh except:
#   1. Calls run_em_algorithm_llm_only.py instead of run_em_algorithm.py
#   2. No sentence-transformer embeddings are used
...
#      0.65 cosine sim ≈ 0.15 confusion similarity for "similar enough
#      to merge" in practice.
#
# Output goes to em_runs_llm_only/ to keep it separate.
###############################################################################
