...
class SchemaRefinementConfig:
    ...
    split_cohesion_max: float = 0.60  # clusters with cohesion ABOVE this are routed to revise, not split
    ...
    merge_posterior_cosine_min: float = 0.85  # cosine similarity of mean posterior vectors for merge detection
    ...
    revise_cohesion_min: float = 0.60  # clusters must have cohesion AT LEAST this high to revise (coherent but mislabeled)
...
def compute_cluster_metrics(
    ...
    for cluster_id, group in cluster_iterator:
        ...
        # Cohesion: exp(-mean_KL) from group mean posterior.
        # High cohesion = documents agree with each other (even if they disagree with the label).
        cohesion = kl_divergence_cohesion(pzx_subset)

        # Mean posterior vector (centroid of p(z|x) across cluster documents).
        # Used downstream for merge detection via cosine similarity.
        mean_posterior = pzx_subset.mean(axis=0)  # shape (K,)
        ...
        logging.debug(
            "Cluster %s ('%s', size %s, mapped %s): L_z=%.4f, C_z=%.4f, V_z=%.4f, cohesion=%.4f",
            cluster_id, label, len(indices), len(valid_indices), L_z, C_z, V_z, cohesion,
        )
...
def decide_schema_updates(
    ...
    # Clusters that pass split distress conditions but have HIGH cohesion
    # (coherent-but-mislabeled) are redirected to revise instead.
    split_redirected_to_revise: set[int] = set()
    ...
    for cid in valid_cluster_ids:
        ...
        split_candidate_rows.append(
            {
                ...
                "passes": split_passes,
                "redirected_to_revise": bool(cid in split_redirected_to_revise),
                "cohesion": cohesion,
                "score": split_score,
            }
        )
        ...
        logging.info(
            "Cluster %s ('%s'): L_z=%.4f, C_z=%.4f, V_z=%.4f, cohesion=%.4f, size=%s. "
            "remove_thresh=%.4f, split_size_min=%s, split_q_thresholds=[L<=%.4f, C<=%.4f, V>=%.4f], "
            "split_cohesion_max=%.4f, gap_median=%.4f, runner_up_share=%.4f",
            cid,
            m.get('label', 'N/A'),
            m['L_z'],
            m['C_z'],
        ...
        if split_enabled:
            ...
            if split_passes:
                # Cohesion-based routing: high cohesion → mislabeled (revise),
                # low cohesion → genuinely incoherent (split).
                if cohesion > config.split_cohesion_max:
                    # Cluster is distressed but internally coherent → mislabeled, redirect to revise.
                    split_redirected_to_revise.add(cid)
                    logging.info(
                        f"  Cluster {cid} ('{m.get('label', 'N/A')}'): passes split conditions "
                        f"(structural={structural_names} quality={passed_names}) but cohesion "
                        f"{cohesion:.4f} > split_cohesion_max {config.split_cohesion_max:.4f} → "
                        f"REDIRECTING to revise (coherent but likely mislabeled)."
                    )
                    # Mark as not passing split (it's redirected)
                    split_passes = False
                else:
                    logging.info(
                        f"  Suggest SPLIT for cluster {cid} ('{m.get('label', 'N/A')}'): "
                        f"structural={structural_names} quality={passed_names} "
                        f"runner_up_share={runner_up_share:.3f} gap_median={gap_median:.3f} "
                        f"cohesion={cohesion:.4f} <= {config.split_cohesion_max:.4f} score={split_score:.4f}."
                    )
    ...
    # --- MERGE (LLM-only: confusion matrix + posterior cosine similarity) ---
    top_pairs = []
    ...
    # Always exclude clusters redirected to revise — they're coherent, not merge candidates.
    if independent_operation_proposals:
        eligible_for_merge_ids = [
            cid for cid in valid_cluster_ids
            if cid not in split_redirected_to_revise
        ]
    else:
        eligible_for_merge_ids = [
            cid
            for cid in valid_cluster_ids
            if cid not in actions['remove'] and cid not in actions['split'] and cid not in split_redirected_to_revise
        ]
    ...
    if not merge_enabled:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                # Posterior cosine similarity: how similar the average posterior
                # profiles of the two clusters are, regardless of label assignment.
                # This catches spurious splits where the LLM validates the
                # label distinction (low confusion) but documents are similar.
                mp_i = m_i.get('mean_posterior')
                mp_j = m_j.get('mean_posterior')
                if mp_i is not None and mp_j is not None:
                    norm_i = np.linalg.norm(mp_i)
                    norm_j = np.linalg.norm(mp_j)
                    if norm_i > 1e-12 and norm_j > 1e-12:
                        posterior_cosine = float(np.dot(mp_i, mp_j) / (norm_i * norm_j))
                    else:
                        posterior_cosine = 0.0
                else:
                    posterior_cosine = 0.0

                if log_top_pairs > 0:
                    top_pairs.append((sim_val, posterior_cosine, id_i, id_j))
                ...
                # Posterior cosine can independently trigger a merge (catches spurious splits)
                posterior_cosine_passes = posterior_cosine >= config.merge_posterior_cosine_min
                merge_passes = (
                    (
                        (sim_margin > 0.0)
                        and (c_margin > 0.0)
                ...
                merge_score = float(
                    ...
                    + (2.0 if posterior_cosine_passes else 0.0)
                )
                merge_candidate_rows.append(
                    {
                        ...
                        "sim": float(sim_val),
                        "posterior_cosine": float(posterior_cosine),
                        "posterior_cosine_passes": bool(posterior_cosine_passes),
                        "diff_L": float(diff_L),
                        "diff_C": float(diff_C),
        ...
        for r in passing_merge_rows:
            ...
            trigger_str = []
            if r["sim_margin"] > 0.0:
                trigger_str.append(f"confusion={r['sim']:.4f}")
            if r.get("posterior_cosine_passes"):
                trigger_str.append(f"posterior_cosine={r['posterior_cosine']:.4f}")
            logging.info(
                "  Suggest MERGE for clusters %s ('%s') and %s ('%s'): "
                "triggers=%s, partner_support=%.4f (%s), label_overlap=%.4f, min_size=%s, C_diff=%.4f, "
                "L_diff(optional)=%.4f, score=%.4f.",
                id_i,
                r["label_i"],
                id_j,
                r["label_j"],
                " + ".join(trigger_str) if trigger_str else "none",
    ...
    # --- REVISE (with cohesion-based routing from split) ---
    ...
    if not revise_enabled:
        ...
        # Even if revise is disabled, warn about redirected clusters that can't be handled.
        if split_redirected_to_revise:
            logging.warning(
                "  %d cluster(s) redirected from split to revise due to high cohesion, "
                "but revise is disabled: %s",
                len(split_redirected_to_revise),
                sorted(split_redirected_to_revise),
            )
        ...
        for cid in eligible_for_revise:
            ...
            # Clusters redirected from split already passed distress conditions
            # AND have high cohesion — they are strong revise candidates.
            if cid in split_redirected_to_revise:
                if not in_cooldown:
                    actions['revise'].append(cid)
                    logging.info(
                        "  Suggest REVISE for cluster %s ('%s'): redirected from split "
                        "(cohesion %.4f > split_cohesion_max %.4f, coherent but mislabeled).",
                        cid,
                        m.get('label', 'N/A'),
                        cohesion,
                        config.split_cohesion_max,
                    ...
                    logging.info(
                        "  Cluster %s ('%s'): redirected from split to revise but in cooldown (until iter %s).",
                        cid, m.get('label', 'N/A'), cooldown_until,
                    )
                continue

            # Standard revise path: require cohesion >= revise_cohesion_min
            # to confirm the cluster is coherent but just has a bad label.
            revise_conditions = [
            ...
            if (
                (not in_cooldown)
                and cohesion >= config.revise_cohesion_min
                and int(sum(bool(x) for x in revise_conditions)) >= max(1, int(config.revise_min_conditions))
            ):
                actions['revise'].append(cid)
                logging.info(
                    "  Suggest REVISE for cluster %s ('%s'): passed %s/%s revise conditions, "
                    "cohesion %.4f >= revise_cohesion_min %.4f.",
                    cid,
                    m.get('label', 'N/A'),
                    sum(bool(x) for x in revise_conditions),
                    len(revise_conditions),
                    cohesion,
                    config.revise_cohesion_min,
                )
            elif (
                (not in_cooldown)
                and cohesion < config.revise_cohesion_min
                ...
                logging.info(
                    "  Cluster %s ('%s'): passes revise distress conditions (%s/%s) but "
                    "cohesion %.4f < revise_cohesion_min %.4f — skipping revise (incoherent, may need split).",
                    cid,
                    m.get('label', 'N/A'),
                    sum(bool(x) for x in revise_conditions),
                    len(revise_conditions),
    ...
    if log_top_pairs > 0 and top_pairs:
        # Sort by max of confusion and posterior cosine for most informative ordering
        top_pairs.sort(reverse=True, key=lambda x: max(x[0], x[1]))
        logging.info(f"Top-{log_top_pairs} merge candidate similarities:")
        for sim_val, post_cos, id_i, id_j in top_pairs[:log_top_pairs]:
            m_i = cluster_metrics[id_i]
            m_j = cluster_metrics[id_j]
            diff_L = abs(m_i['L_z'] - m_j['L_z'])
            diff_C = abs(m_i['C_z'] - m_j['C_z'])
            logging.info(
                f"  pair ({id_i}, {id_j}) confusion={sim_val:.4f} posterior_cosine={post_cos:.4f} "
...
def main(args):
    ...
    if args.num_iterations > 0:
        ...
        config = SchemaRefinementConfig(
            ...
            revise_cohesion_min=args.revise_cohesion_min,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split_cohesion_max", type=float, default=0.60, help="Clusters with cohesion ABOVE this are routed to revise instead of split (coherent but mislabeled).")
    ...
    parser.add_argument("--merge_posterior_cosine_min", type=float, default=0.85, help="Cosine similarity of mean posterior vectors to trigger merge (catches spurious splits).")
    parser.add_argument(
    ...
    parser.add_argument("--revise_cohesion_min", type=float, default=0.60, help="Clusters must have cohesion at least this high to qualify for revise (coherent but mislabeled).")
